...
class FakeDataGenerator
{
    ...
    public function generateManga(array $authors, array $tags): Manga
    {
        ...
        // Add random authors
        $authorCount = min(rand(1, 2), count($authors));
        foreach ($this->faker->randomElements($authors, $authorCount) as $author) {
            $manga->addAuthor($author);
        }

        // Add random artists (can be same as authors)
        $artistCount = min(rand(1, 2), count($authors));
        foreach ($this->faker->randomElements($authors, $artistCount) as $artist) {
            $manga->addArtist($artist);
        }

        // Add random tags
        $tagCount = min(rand(2, 5), count($tags));
        foreach ($this->faker->randomElements($tags, $tagCount) as $tag) {
            $manga->addTag($tag);
        }
    ...
    public function generateCustomList(User $owner, array $manga): CustomList
    {
        ...
        $mangaCount = min(rand(1, 5), count($manga));
        foreach ($this->faker->randomElements($manga, $mangaCount) as $mangaItem) {
